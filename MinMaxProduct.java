...
public class MinMaxProduct {
    ...
    public static void main(String[] args) {
        ...
        for(int mat_i=0;mat_i<n;mat_i++)
        {
            ...
            {
                max=mat1[mat_i][mat_j];
                for(int mat_k=0;mat_k<n;mat_k++)
                {
                    if(max>mat2[mat_k][mat_j] && max>mat1[mat_i][mat_k])
                    {
                        continue;
                    }
                    if(mat2[mat_k][mat_j]>mat1[mat_i][mat_k] &&  mat1[mat_i][mat_k]>max)
                    {
                        max=mat1[mat_i][mat_k];
                    }
                    else if(mat2[mat_k][mat_j]<mat1[mat_i][mat_k] && mat2[mat_k][mat_j]>max)
                    {
                        max=mat2[mat_k][mat_j];
                    }
                }
